    ...
    def __init__(self, parent):
        ...
        for i in button_layout:
            if i != "=" or "0" or ".":
                ...
                col = col + 1
            if i == "=":
                Button(parent, text = i, command = lambda i=i: self.enterButton(i), bg = "yellow", padx = PX, pady = PY).grid(row = 5, column = 4, columnspan = 2)

            if i == "0":
                Button(parent, text = i, command = lambda i=i: self.enterButton(i), bg = "yellow", padx = PX, pady = PY).grid(row = 5, column = 0, columnspan = 2, sticky = W + E)
            if i == ".":
                Button(parent, text = i, command = lambda i=i: self.enterButton(i), bg = "yellow", padx = PX, pady = PY).grid(row = 5, column = 2)
